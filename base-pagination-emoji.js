...
function renderHTML(colo, timestamp, path) {
  ...
  try {
    ...
    // Render files with dynamic icons
    var html = '';
    for (var i = 0; i < data.files.length; i++) {
      ...
      if (isFolder) {
        ...
        if (/\.(mp4|webm|avi|mpg|mpeg|mkv|mov|wmv|ts|flv)$/i.test(lowerName)) {
          icon = '🎬'; // Video
        } else if (/\.(bmp|jpg|jpeg|png|gif)$/i.test(lowerName)) {
          icon = '🖼️'; // Photo
        } else if (/\.(mp3|m4a|wav|flac|aac|ogg)$/i.test(lowerName)) {
          icon = '🎵'; // Audio
        } else if (/\.(apk)$/i.test(lowerName)) {
          icon = '📱'; // APK
        } else if (/\.(exe|msi)$/i.test(lowerName)) {
          icon = '🖥️'; // Windows executables
